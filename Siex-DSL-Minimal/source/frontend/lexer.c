  ...
  switch (length) {
    ...
    case 4:
      ...
      if (strncmp(start, "impl", 4) == 0)
        return TOK_KEYWORD_IMPL;
      if (strncmp(start, "main", 4) == 0)
        return TOK_KEYWORD_MAIN;
...
token lexer_next_token(lexer *lex) {
  ...
  switch (c) {
    ...
    case ':':
      lexer_next_char(lex);
      return make_token(TOK_COLON, start, 1, token_location);
    default:
      break;
  }
