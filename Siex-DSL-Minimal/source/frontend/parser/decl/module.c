...
int parse_module_decl(parser *p) {
  ...
  siex_module* module = arena_new_zero(p->memory, siex_module);

  module->name = name;

  vector_init_t(&module->needs, siex_need*);
  ...
  // guardar módulo
  vector_push(p->memory, &p->program.modules, &module);
...
static int parse_need_decl(parser* p, siex_module* module) {
  ...
  char* first = arena_strdup(p->memory, t.token_start, t.length);
  ...
  if (t.type == TOK_COLON) {
    ...
    if (parser_peek(p).type != TOK_COLON) {
      parser_error(parser_peek(p), "Expected '::'");
      return 1;
    }
    ...
    if (t2.type != TOK_IDENTIFIER) {
      parser_error(t2, "Expected identifier after '::'");
      return 1;
    }

    alias = first;
    name = arena_strdup(p->memory, t2.token_start, t2.length);
  ...
  siex_need* need = arena_new_zero(p->memory, siex_need);
  need->alias = alias;
  need->name = name;
  ...
  // acá entra vector
  vector_push(p->memory, &module->needs, &need);
