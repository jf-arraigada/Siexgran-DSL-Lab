...
int parse_backend_decl(parser *p) {
  ...
  siex_backend* backend = arena_new_zero(&p->mem, siex_backend);
  backend->name = name;
  ...
  vector_push(&p->mem, &p->program.backends, &backend);
  return 0;
}
...
static int parse_impl_decl(parser* p, siex_backend* backend) {
  ...
  siex_impl* impl = arena_new_zero(&p->mem, siex_impl);
  impl->name = name;
  ...
  vector_push(&p->mem, &backend->implementations, &impl);
